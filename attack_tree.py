...
def create_attack_tree_prompt(methodology, app_type, authentication, internet_facing, sensitive_data, pam, app_input):
...
and additional details provided to you to produce an attack tree. The attack tree should reflect the potential threats for 
the application based on the details given.
...
def get_attack_tree_openai(api_key, model_name, prompt):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": """
You are a helpful assistant designed to output attack tree.
...
IMPORTANT: Round brackets are special characters in Mermaid syntax. If you want to use round brackets inside a node label you 
MUST wrap the label in double quotes. For example, ["Example Node Label (ENL)"]."""
},
